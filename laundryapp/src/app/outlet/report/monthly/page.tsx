  ...
  useEffect(() => {
    async function fetchReport() {
      try {
        ...
        const response = await fetch("/api/outlets/reports/monthly");
        ...
        const filteredTransactions = data.transaction.filter(
          (trans: Transaction) => {
        ...
        filteredTransactions.reduce(
          (acc: number, trans: Transaction) => acc + trans.totalAmount,
          0
        );
        const totalPrice = filteredTransactions.reduce(
          (acc: number, trans: Transaction) => {
            return (
              acc +
              trans.serviceDetail.reduce(
                (sum: number, service: ServiceDetail) => sum + service.price,
                0
              )
            );
          },
        ...
        setReportData({
          ...
          totalTransaction: filteredTransactions.length,
